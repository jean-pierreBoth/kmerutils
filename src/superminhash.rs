...
//! (as in finch module). In the other case Fnv (fast when hashing small values as integer according to documentation) ,
...
/// or fxhash can be used.
/// This type contains values of sketch and vector utilities used to compute them.
/// values that we want to sketch must implement Hash.
/// 
...
impl <'a, T:Hash ,  H : 'a + Hasher+Default> SuperMinHash<'a,T, H> {
    ...
    /// size is size of sketch. build_hasher is the build hasher for the type of H we want.
    ...
    /// Reinitialize minhasher, keeping size of sketches. SuperMinHash can then be reinitialized and used again with sketch_slice.
    ///
